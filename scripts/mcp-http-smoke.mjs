...
function ensureMcpHttpBinary() {
  const cargoTargetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'mini-term-mcp-http-target-'));
  runOrThrow(
    'cargo',
    ['build', '--manifest-path', tauriManifestPath(), '--bin', 'mini-term-mcp-http'],
    {
      env: {
        ...process.env,
  ...
  const builtBinaryPath = path.join(
    cargoTargetDir,
    'debug',
    binaryFileName('mini-term-mcp-http'),
  );
  assert.ok(fs.existsSync(builtBinaryPath), `missing MCP HTTP binary: ${builtBinaryPath}`);
...
function createHttpClient(binaryPath, env, port) {
  ...
  child.on('exit', (code) => {
    if (!shuttingDown && code !== null && code !== 0) {
      console.error(`mini-term-mcp-http exited with code ${code}\n${stderr}`);
    }
  });

  async function request(methodName, params = {}, options = {}) {
    const payload = options.notification
      ? { jsonrpc: '2.0', method: methodName, params }
      : { jsonrpc: '2.0', id: nextId++, method: methodName, params };
    let response;
    try {
      ...
      throw new Error(
        [
          `HTTP MCP request failed: ${methodName}`,
          `reason: ${reason}`,
          stderr.trim() ? `stderr:\n${stderr.trim()}` : '',
        ]
          .filter(Boolean)
  ...
  async function initialize() {
    const response = await request('initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'mcp-http-smoke', version: '0.0.0' },
    });
...
async function waitForHttpHealth(port) {
  const deadline = Date.now() + WAIT_TIMEOUT_MS;
  while (Date.now() < deadline) {
    try {
      const response = await httpRequest({ port, method: 'GET', pathName: '/health' });
      if (response.statusCode === 200) {
  ...
  throw new Error('HTTP MCP server never became healthy');
}
...
async function main() {
  const stagedBinaryPath = ensureMcpHttpBinary();

  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'mini-term-mcp-http-smoke-'));
  ...
  try {
    ...
    fs.rmSync(path.dirname(stagedBinaryPath), { recursive: true, force: true });
  }
}
