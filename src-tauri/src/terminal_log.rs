...
pub fn append_pty_output(app: &AppHandle, pty_id: u32, data: &str) {
    ...
    try_enqueue_event(sender, event);
}

fn try_enqueue_event(sender: &SyncSender<TerminalLogEvent>, event: TerminalLogEvent) {
    // PTY reader 热路不能因日志背压阻塞或同步写 stderr。
...
mod tests {
    ...
    #[test]
    fn queue_backpressure_and_disconnect_are_ignored() {
        ...
        try_enqueue_event(
            ...
            TerminalLogEvent {
                pty_id: 1,
                data: "dropped while full".to_string(),
            },
        );

        assert_eq!(receiver.try_recv().unwrap().data, "first");
        drop(receiver);
        ...
        try_enqueue_event(
            ...
            TerminalLogEvent {
                pty_id: 1,
                data: "dropped after disconnect".to_string(),
            },
        );
    }
