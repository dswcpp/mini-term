...
async function persistSettingsConfig(
  ...
  try {
    await saveConfigOptimistic(nextConfig);
    return true;
  } catch (error) {
    await showAlert(
      translate('settings.common.saveFailedTitle'),
      formatError(error),
    );
    return false;
  }
...
export function TerminalSettings() {
  ...
  }, [savedLineThreshold, savedCharThreshold, savedTerminalLogPath, savedTerminalLogMaxSizeMb]);
  ...
  const save = useCallback(async (updatedShells: ShellConfig[], updatedDefault: string) => {
    ...
    await persistSettingsConfig(newConfig, t);
  }, [t]);
  ...
  const handleChooseTerminalLogPath = async () => {
    const selected = await saveDialog({
      ...
      filters: [{ name: t("settings.terminal.logFileFilter"), extensions: ['log', 'txt'] }],
    });
  ...
  const commitTerminalLogMaxSize = () => {
    ...
    const clamped = Number.isFinite(n) && n > 0 ? Math.min(Math.max(n, 1), 10240) : savedTerminalLogMaxSizeMb;
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between gap-3 px-3 py-2.5 rounded-[var(--radius-md)] bg-[var(--bg-base)] border border-[var(--border-subtle)]">
        ...
        <div className="flex items-center gap-2 min-w-0 flex-[1.4]">
          ...
          <button
            type="button"
            className="px-3 py-1 text-base bg-[var(--bg-elevated)] text-[var(--text-secondary)] border border-[var(--border-default)] rounded-[var(--radius-sm)] hover:border-[var(--accent)] hover:text-[var(--accent)] transition-all flex-shrink-0"
            onClick={handleChooseTerminalLogPath}
          >
            ...
          </button>
        </div>
      ...
      <div className="flex items-center justify-between px-3 py-2.5 rounded-[var(--radius-md)] bg-[var(--bg-base)] border border-[var(--border-subtle)]">
        ...
        <button
          ...
          onClick={() => void saveConfigPatch({ smartCopyPaste: !smartCopyPasteEnabled })}
        >
          <span
            className={`absolute top-0.5 left-0 w-4 h-4 rounded-full bg-white transition-transform ${
...
export function SystemSettings() {
  ...
  const handleAddEditor = useCallback(() => {
    ...
    if (editors.some((e) => e.name === trimmedName)) {
      void showAlert(
        t("settings.common.validationFailedTitle"),
        t("settings.system.editorExistsAlert", { name: trimmedName }),
      );
      return;
    }
  ...
  const handleUpdateEditor = useCallback((idx: number, editor: EditorConfig) => {
    ...
    if (editor.name !== oldName && editors.some((e, i) => i !== idx && e.name === editor.name)) {
      void showAlert(
        t("settings.common.validationFailedTitle"),
        t("settings.system.editorExistsAlert", { name: editor.name }),
      );
      return;
    }
  ...
  const handleThemeChange = useCallback((theme: 'auto' | 'light' | 'dark') => {
    ...
    void persistSettingsConfig(newConfig, t);
    applyTheme(theme);
...
export function AiHookSettings() {
  const t = useT();
  const config = useAppStore((s) => s.config);
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between px-3 py-2.5 rounded-[var(--radius-md)] bg-[var(--bg-base)] border border-[var(--border-subtle)]">
        ...
        <button
          ...
          onClick={() => void saveConfigPatch({ aiCompletionPopup: !config.aiCompletionPopup })}
        >
          <span
            className={`absolute top-0.5 left-0 w-4 h-4 rounded-full bg-white transition-transform ${
      ...
      <div className="flex items-center justify-between px-3 py-2.5 rounded-[var(--radius-md)] bg-[var(--bg-base)] border border-[var(--border-subtle)]">
        ...
        <button
          ...
          onClick={() => void saveConfigPatch({ aiCompletionTaskbarFlash: !config.aiCompletionTaskbarFlash })}
        >
          <span
            className={`absolute top-0.5 left-0 w-4 h-4 rounded-full bg-white transition-transform ${
      ...
      <div className="flex items-center justify-between px-3 py-2.5 rounded-[var(--radius-md)] bg-[var(--bg-base)] border border-[var(--border-subtle)]">
        ...
        <button
          ...
          onClick={() => void saveConfigPatch({ aiCompletionSound: !config.aiCompletionSound })}
        >
          <span
            className={`absolute top-0.5 left-0 w-4 h-4 rounded-full bg-white transition-transform ${
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            ...
            {config.aiCompletionSoundPath && (
              <button
                ...
                onClick={() => void saveConfigPatch({ aiCompletionSoundPath: undefined })}
              >
