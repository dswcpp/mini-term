...
assert.equal(
  buildSshCommand({ ...base, user: ' deploy ', host: ' example.com ' }),
...
assert.equal(
  buildSshCommand(base, '/tmp/prod"key'),
  'ssh -i "/tmp/prod\\"key" -o IdentitiesOnly=yes deploy@example.com',
);

assert.deepEqual(
  validateSshConnectionTarget({ user: '', host: 'example.com', port: 22 }),
  { ok: false, reason: 'missing-user' },
);

assert.deepEqual(
  validateSshConnectionTarget({ user: 'deploy', host: '  ', port: 22 }),
...
for (const port of [NaN, 0, -1, 22.5, 65536]) {
  assert.deepEqual(
    validateSshConnectionTarget({ user: 'deploy', host: 'example.com', port }),
    { ok: false, reason: 'invalid-port' },
  );
  assert.throws(
    () => buildSshCommand({ ...base, port }),
    /invalid-port/,
  );
}
